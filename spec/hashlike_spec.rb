...
describe Gorillib::Hashlike do
  ...
  BASE_HSH_WITH_ARRAY_VALS = {:a => [300,333], :b => [400,444], :e => 500,  [1, 2, [3, 4]] => [1, [2, 3, [4, 5, 6]]] }.freeze
  BASE_HSH_WITH_ARRAY_KEYS = {[:a,:b] => 3, [:c, :d] => 4, :e => 5,  [1, 2, [3, 4]] => [1, [2, 3, [4, 5, 6]]] }.freeze
  ...
  describe '#delete' do
    ...
    it 'does not include? key after deleting' do
      ...
      @hshlike.should include(:nil_val)
      @hshlike.delete(:a)
      @hshlike.delete(:nil_val)
      @hshlike.should_not include(:a)
      @hshlike.should_not include(:nil_val)
    end
  end
  ...
  describe '#values_at' do
    ...
    it 'returns duplicate keys or missing keys in given slot' do
      @hshlike.values_at(:b, :b, :i_am_missing, :nil_val, 'c', :c).should == [200, 200, nil, nil, 300, nil]
    end
  end
  ...
  shared_examples_for :merging_method do |method_to_test|
    ...
    describe 'with a block' do
      ...
      it 'passes params in the order key, current val, other hash val' do
        seen_args = []
        ret_val = @hshlike.send(method_to_test, {:a => "aaa", :nil_val => "here", :new_key => "zzz" }) do |key, other_val, hsh_val|
  ...
  describe '#flatten' do
    it 'with no arg returns a one-dimensional flattening' do
      ...
      ret_val.should == [  :a, [300, 333],   :b, [400, 444],   :e, 500,   [1, 2, [3, 4]], [1, [2, 3, [4, 5, 6]]] ]
